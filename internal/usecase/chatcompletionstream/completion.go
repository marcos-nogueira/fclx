...
import (
	...
	"github.com/marcos-nogueira/fclx/chatservice/internal/domain/gateway"
...
type ChatCompletionConfigInputDTO struct {
	...
	Stop                 []string
	MaxTokens            int
	PresencePanalty      float32
	FrequencyPanalty     float32
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error) {
	...
	resp, err := uc.OpenAiClient.CreateChatCompletionStream(
		...
		openai.ChatCompletionRequest{
			...
			PresencePenalty:  chat.Config.PresencePanalty,
			FrequencyPenalty: chat.Config.FrequencyPanalty,
...
func createNewChat(input ChatCompletionInputDTO) (*entity.Chat, error) {
	...
	chatConfig := &entity.ChatConfig{
		...
		PresencePanalty:  input.Config.PresencePanalty,
		FrequencyPanalty: input.Config.FrequencyPanalty,
		Model:            model,
	}
